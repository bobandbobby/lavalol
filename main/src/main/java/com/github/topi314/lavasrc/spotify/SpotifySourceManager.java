...
public class SpotifySourceManager
    ...
    private static final String RECOMMENDATIONS_PREFIX = "sprec:";
    private static final String PREVIEW_PREFIX = "spprev:";
    private static final long PREVIEW_LENGTH = 30000;
    private static final String SHARE_URL = "https://spotify.link/";
    ...
    private static final Set<AudioSearchResult.Type> SEARCH_TYPES = Set.of(
    ...
    public SpotifySourceManager(String[] providers, String countryCode, Function<Void, AudioPlayerManager> audioPlayerManager) {
        this(countryCode, audioPlayerManager, new DefaultMirroringAudioTrackResolver(providers));
    }
    ...
    public SpotifySourceManager(String countryCode, Function<Void, AudioPlayerManager> audioPlayerManager, MirroringAudioTrackResolver mirroringAudioTrackResolver) {
    ...
    private AudioSearchResult getAutocomplete(String query, Set<AudioSearchResult.Type> types) throws IOException {
        ...
        var albums = json.get("albums").get("items").values().stream()
                .map(album -> new SpotifyAudioPlaylist(
                        album.get("name").text(),
        ...
        var artists = json.get("artists").get("items").values().stream()
                .map(artist -> new SpotifyAudioPlaylist(
        ...
        var playlists = json.get("playlists").get("items").values().stream()
                .map(playlist -> new SpotifyAudioPlaylist(
    ...
    public AudioItem getSearch(String query, boolean preview) throws IOException {
        ...
        if (artistJson != null) {
            json.get("tracks").get("items").values().forEach(track -> {
                var artistId = track.get("artists").index(0).get("id").text();
                var artist = artistJson.get("artists").values().stream()
                        .filter(a -> a.get("id").text().equals(artistId))
    ...
    public AudioItem getAlbum(String id, boolean preview) throws IOException {
        ...
        var pages = 0;
        do {
            page = this.getJson(API_BASE +
                    "albums/" + id +
                    "/tracks?limit=" + ALBUM_MAX_PAGE_ITEMS +
                    "&offset=" + offset);
            offset += ALBUM_MAX_PAGE_ITEMS;

            var tracksPage = this.getJson(API_BASE +
                    "tracks/?ids=" + page.get("items").values().stream()
    ...
    public AudioItem getPlaylist(String id, boolean preview) throws IOException {
        ...
        do {
            page = this.getJson(API_BASE +
                    "playlists/" + id +
                    "/tracks?limit=" + PLAYLIST_MAX_PAGE_ITEMS +
                    "&offset=" + offset);
            offset += PLAYLIST_MAX_PAGE_ITEMS;

            tracks.addAll(this.parseTracks(page, preview));
        ...
        return new SpotifyAudioPlaylist(
                ...
                json.get("tracks").get("total").asInt(0)
        );
    }

    public AudioItem getArtist(String id, boolean preview) throws IOException {
        var json = this.getJson(API_BASE + "artists/" + id + "/top-tracks?country=" + this.countryCode);
        if (json == null) {
            return AudioReference.NO_TRACK;
        }

        var artistJson = this.getJson(API_BASE + "artists/" + id);
        if (artistJson == null) {
            artistJson = JsonBrowser.newMap();
        }

        json.get("tracks").values().forEach(track -> track.get("artists").index(0).put("images", artistJson.get("images")));
    ...
    public List<AudioTrack> parseTracks(JsonBrowser json, boolean preview) {
        return json.get("tracks").values().stream()
                .map(track -> this.decodeTrack(
    ...
    public AudioTrack decodeTrack(AudioTrackInfo trackInfo, JsonBrowser track, boolean preview) {
        if (track == null) {
            return null;
        }

        return this.decodeTrack(
                trackInfo,
                preview ? track.get("preview_url").text() : null
        );
    }

    @Override
    public void configureRequests(Consumer<RequestConfig.Builder> configurator) {
